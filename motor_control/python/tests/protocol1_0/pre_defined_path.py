    ...
    try:
        ...
        while True:
            ...
            for i in range(len(target)-1):
                ...
                trajectory_angles[i].append(modified_sine_trajectory(target[i],target[i+1]))

            for location in range(len(trajectory_angles)):
                for step in trajectory_angles[location]:
                    for angle in step:
                        move_to_location(angle)
